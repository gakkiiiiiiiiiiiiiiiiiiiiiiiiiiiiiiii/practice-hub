<template>
	<view class="wrong-page">
		...
		<!-- 错题列表 -->
		<scroll-view class="wrong-list" scroll-y @refresherrefresh="onRefresh" refresher-enabled>
			<!-- 加载状态 -->
			...
			<!-- 空状态 -->
			<view v-else-if="wrongList.length === 0" class="empty-state">
				<text class="empty-icon">📚</text>
				<text class="empty-text">暂无错题</text>
				<text class="empty-hint">答错的题目会自动加入错题集</text>
			</view>
				...
				>
					...
					<view class="item-content">
						...
						<view class="item-meta">
							<text class="meta-item">错{{ item.error_count }}次</text>
							<text class="meta-divider">·</text>
							<text class="meta-item">{{ formatTime(item.last_error_time) }}</text>
...
import { getWrongBookList, removeWrongQuestion } from '@/api/index';
...
const loadWrongList = async () => {
	...
	try {
		...
		// 注意：后端在删除题目时会自动级联删除错题集记录，这里只是作为安全过滤
		data = data.filter((item) => {
			return item.question !== null && item.question !== undefined;
		});

		wrongList.value = data.map((item) => {
			// 查找课程名称
			const course = courseList.value.find((c) => c.id === item.course_id);
			return {
				...item,
...
const handleReview = (item) => {
	...
	if (!item.question || item.question === null) {
		...
		// 自动从列表中移除
		wrongList.value = wrongList.value.filter((i) => i.id !== item.id);
		return;
	}

	uni.navigateTo({
		url: `/pages/answer/index?mode=practice&questionId=${item.question_id}&from=wrong`,
	});
};
...
const handleRemove = async () => {
	...
	try {
		...
		await removeWrongQuestion({ id: currentItem.value.id });
		uni.hideLoading();
